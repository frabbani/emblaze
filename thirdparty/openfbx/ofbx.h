...
// https://github.com/nem0/OpenFBX/blob/master/README.md
...
struct Vec2Attributes {
  ...
  Vec2Attributes(const Vec2 *v, const int *i, int c)
      :
      values(v),
      indices(i),
      count(c) {
  ...
  Vec2 get(int i) const {
    return indices ? values[indices[i]] : values[i];
  }
};
...
struct Vec3Attributes {
  ...
  Vec3Attributes(const Vec3 *v, const int *i, int c, int vc)
      :
      values(v),
      indices(i),
      count(c),
...
struct Vec4Attributes {
  ...
  Vec4Attributes(const Vec4 *v, const int *i, int c)
      :
      values(v),
      indices(i),
      count(c) {
